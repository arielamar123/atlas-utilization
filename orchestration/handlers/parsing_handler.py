...
def select_metadata_for_parsing(metadata, release_years, parse_mc):
    """Select data or MC keys explicitly without changing schema names."""
    requested = list(release_years or [])
    if requested:
        selected_keys = set()
        for release in requested:
            if release.startswith("record_"):
                ...
                continue
            base = release[:-3] if release.endswith("_mc") else release
            selected_keys.add(f"{base}_mc" if parse_mc else base)
        return {key: value for key, value in metadata.items() if key in selected_keys}

    # Specific CMS record keys are not paired data/MC releases and remain
    # eligible regardless of the ATLAS MC selector.
    return {
        key: value
